# Codex E59 - Financials Phase B, batch 3 of 4

Company research over the 169 Financials companies that Phase A unblocked, in four
stratified batches. This is batch 3: **42 companies**, request_id
`9e243401d2fb0d1e380e`, depth 3, citations required. The roster was checked against the
store before staging: none of these 42 companies had already been researched.
...
batch. Do not evaluate E58 arm B after batch 3; three of four batches is not the full
Financials Phase B version scope.

## Prompt

`````
# COMPANY LAB - E59 PHASE B, FINANCIALS BATCH 3 OF 4
...
Company research. The request file is frozen and is the authority on what to research:
  ...
  request_id  9e243401d2fb0d1e380e
  42 companies, depth 3, citations REQUIRED
...
## THE ROSTER, by industry
  ...
  regional_banking                         11  AUB BBT CFR EGBN FITB HAFC IBOC PNFP SBSI TRMK WAFD
  asset_management_custody_banks            4  AMG BLK HLNE TROW
  investment_banking_brokerage              4  BGC HOOD MC RJF
  consumer_finance                          4  ALLY COF EZPW SLM
  diversified_banks                         2  C USB
  financial_exchanges                       2  COIN VIRT
  financial_data_ratings                    2  DFIN MSCI
  insurance_brokers                         2  AON MRSH
  multi_line_insurance                      2  AIG KMPR
  pc_insurance                              2  AGO RLI
  alternative_asset_management              1  BX
  climate_infrastructure_finance             1  HASI
  custody_banks                              1  BNY
  life_insurance                             1  MET
  mortgage_reits                            1  EFC
  private_mortgage_insurance                1  ESNT
  reinsurance                               1  MTG
...
This is batch 3 of 4. Do not research companies from the other batches.
...
This field returned UNKNOWN for all 43 companies in Financials batch 1 and all 42 in
batch 2. The field is not unanswerable; it is unanswerable by reading documents. For
regional banks it is computed from consecutive years of FDIC Summary of Deposits data,
using identical geographies, excluding acquisition-sized jumps, and abstaining below
three shared markets or 60% deposit coverage.

For the 11 regional banks here, leave `market_share_direction` UNKNOWN and give the
blocking reason as "computed from FDIC Summary of Deposits; production wiring pending."
Spend the search budget elsewhere.

If you nonetheless encounter a document that genuinely establishes direction while doing
other work, report it. A researched value should not be buried merely because a computed
path exists. For non-bank industries, research the field normally; if it remains UNKNOWN,
name the exact primary disclosure that would settle it. Direct-written premium is the
natural measure for insurers.

This carve-out applies only to `market_share_direction` for regional banks. It does not
change any other field or industry.
...
42 companies at depth 3. They share a regulatory and macro literature heavily - rates,
